...
contract SBTMinter is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    modifier OnlyOrder() {
        require(msg.sender == getOrderAddress(), "Only Order can Call.");
        _;
    }

    function initialize(address _OrderRulesCotntract) public initializer {
        __Ownable_init();
    ...
    function upgradeBulkShipperRules(SBTInfo[] calldata _SBTInfo) external onlyOwner {
        uint256 _length = _SBTInfo.length;
    ...
    function _upgradeShipperRules(SBTInfo calldata _SBTInfo) internal {
        ...
        if (_SBTInfo.tier != 0) {
            require(
                ShipperRequirements[_SBTInfo.tier - 1] < _SBTInfo.requirement,
                "Must be greater than the requirements of the next lower tier."
            );
        }
        if (ShipperRequirements[_SBTInfo.tier + 1] != 0) {
            require(
                ShipperRequirements[_SBTInfo.tier + 1] > _SBTInfo.requirement,
                "Must be less than the requirements of the next higher tier."
            );
        }
    ...
    function _upgradeCarrierRules(SBTInfo calldata _SBTInfo) internal {
        ...
        if (_SBTInfo.tier != 0) {
            require(
                CarrierRequirements[_SBTInfo.tier - 1] < _SBTInfo.requirement,
                "Must be greater than the requirements of the next lower tier."
            );
        }
        if (CarrierRequirements[_SBTInfo.tier + 1] != 0) {
            require(
                CarrierRequirements[_SBTInfo.tier + 1] > _SBTInfo.requirement,
                "Must be less than the requirements of the next higher tier."
            );
    ...
    function getShipperRules() public view returns (SBTInfo[] memory) {
        uint256 _length = ShipperRequirements.length;
        SBTInfo[] memory results = new SBTInfo[](_length);

        for (uint256 i = 0; i < _length; i++) {
            results[i] =
                SBTInfo({tier: i, requirement: ShipperRequirements[i], uri: ShipperTierUri[ShipperRequirements[i]]});
        }
    ...
    function getShipperRuleByTier(uint256 _tier) public view returns (SBTInfo memory) {
        return SBTInfo({
            tier: _tier,
    ...
    function getCarrierRules() public view returns (SBTInfo[] memory) {
        uint256 _length = CarrierRequirements.length;
        SBTInfo[] memory results = new SBTInfo[](_length);

        for (uint256 i = 0; i < _length; i++) {
            results[i] =
                SBTInfo({tier: i, requirement: CarrierRequirements[i], uri: CarrierTierUri[CarrierRequirements[i]]});
        }
    ...
    ///@notice 현재 등록된 SBT 개별 티어의 등급업 조건 및 해당 URI 조회
    function getCarrierRuleByTier(uint256 _tier) public view returns (SBTInfo memory) {
        return SBTInfo({
            tier: _tier,
    ...
    function setShipperSBT(address _owner, address SBTAddress) private {
        ...
        uint256 orderCount = IOrder(getOrderAddress()).getRecord(_owner).completeOrder_shipping;
        for (uint256 i = 0; i < ShipperRequirements.length; i++) {
            uint256 currentStep = ShipperRequirements[i];
            uint256 nextStep = ShipperRequirements[i + 1];
            if (currentStep <= orderCount && orderCount < nextStep || nextStep == 0) {
                uint256 _tokenId = IERC721Enumerable(SBTAddress).tokenOfOwnerByIndex(_owner, 0);
    ...
    function setCarrierSBT(address _owner, address SBTAddress) private {
        ...
        uint256 orderCount = IOrder(getOrderAddress()).getRecord(_owner).completeOrder_carrying;
        for (uint256 i = 0; i < CarrierRequirements.length; i++) {
            uint256 currentStep = CarrierRequirements[i];
            uint256 nextStep = CarrierRequirements[i + 1];
            if (currentStep <= orderCount && orderCount < nextStep || nextStep == 0) {
                uint256 _tokenId = IERC721Enumerable(SBTAddress).tokenOfOwnerByIndex(_owner, 0);
                string memory currentTokenUri = IERC721Metadata(SBTAddress).tokenURI(_tokenId);
    ...
    function getOrderAddress() private view returns (address) {
        return IOrderRules(OrderRulesCotntract).getOrderAddress();
    }
